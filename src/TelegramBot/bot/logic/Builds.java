    ...
    public static String buildsMessage(Map<String, Integer> builds) {
        ...
        while (iterator.hasNext()) {
            ...
            if (!builds.get(temp).equals(0)) {
                tempMessage = ConstantDB.accordanceListOfBuilds.get(temp) + ":    Уровень " + builds.get(temp) + "\n";
    ...
    public String upgradeBuildsMessage(Map<String, Integer> resources, Map<String, Integer> builds) {
        ...
        while (iteratorBuildsKeys.hasNext()) {
            ...
            if (builds.get(temp) + 1 > ConstantResourcesForBuilds.LIST_LIMITS.get(temp)) {
                tempMessage = i + ":      " + ConstantDB.accordanceListOfBuilds.get(temp) + "(Максимум)\n\n";
            } else if (builds.get(temp).equals(0)) {
                continue;
            } else {
                tempMessage = i + ":     " + ConstantDB.accordanceListOfBuilds.get(temp) + "     Уровень " + builds.get(temp) + " -> Уровень " + builds.get(temp) + 1 + "\n";
                tempMessage += "Нужно: " + resourcesNeed.get("Wood") + " дерева, " + resourcesNeed.get("Gold") + " золота и " + resourcesNeed.get("Stone") + " камня\n\n";
            }
            message += tempMessage;
            i++;
        }
    ...
    public String upbuildBuildsMessage(Map<String, Integer> resources, Map<String, Integer> builds) {
        ...
        while (iteratorBuildsKeys.hasNext()) {
            ...
            if (!builds.get(temp).equals(0)) {
                ...
                tempMessage += "Нужно: " + resourcesNeed.get("Wood") + " дерева, "
                        + resourcesNeed.get("Gold") + " золота и "
                        + resourcesNeed.get("Stone") + " камня\n\n";
            }
